...
const Navbar = () => {
  ...
  return (
    <div className=" navbar bg-black px-8 py-6 flex flex-row items-center justify-between">
      <img className="cursor-pointer mx-24 h-12" src={logo} alt="Hoobank Logo" />
      <div className="flex right gap-6">
        <ul className=" flex flex-row justify-evenly items-center gap-4 mx-28">
          {menu_Items.map((item,index) => (
            <ListItems key={item.id} link={item.link} menu={item.menu} name={item.name} />
          ))}
        </ul>
